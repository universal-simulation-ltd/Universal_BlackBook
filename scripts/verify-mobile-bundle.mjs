...
// asset URL comes out as `/blackbook/assets/...`, which on the phone is a 404:
// the module script never runs, `#root` stays empty, and the app is a blank
// screen. Xcode still reports BUILD SUCCEEDED, the icon is right, the version
// is right.
//
// It has happened: Universal PDF 0.6.14 shipped to a real iPhone that way and
// could not load. This script is that repo's guard, copied here so BlackBook
// cannot repeat it.
//
